 ...
 */
const onDidChangeWorkspaceFoldersPromise = () =>
  new Promise((resolve) => {
...
suite('Runner', function () {
  ...
  suite('createRunner()', function () {
    suite('Basic functionality', function () {
      suiteSetup(async function () {
        ...
        await Promise.all([
          ...
          aConfig.update(
            'snippetExcludeSniffs',
            ['ExcludeMe', 'IgnoreThis'],
            ConfigurationTarget.Workspace,
          ),
        ]);
      });
      ...
      suite(
        'No trailing slash needed for "phpSniffer.executablesFolder"',
        function () {
          let beforeValue;
    ...
    suite('Folder resolution', function () {
      const subfolder = Uri.file(
        path.join(__dirname, '/fixtures/config/subfolder'),
      );
      const secondaryFolder = Uri.file(
        path.join(__dirname, '/fixtures/config0'),
      );
    ...
    suite('Automatic detection', function () {
      suite('Executables folder discovered', function () {
        ...
        const subjectUri = Uri.file(
          path.resolve(__dirname, 'fixtures/auto/target'),
        );
      ...
      suite('PHP_CodeSniffer auto-discovers xml ruleset', function () {
        ...
        suiteSetup(async function () {
          ...
          // Create ruleset file.
          const onCreate = fsAsync.writeFile(
            rulesetFile,
            `<?xml version="1.0"?>
<ruleset name="Fixture Ruleset">
  <rule ref="Generic.PHP.LowerCaseConstant"/>
          ...
          const composerInstall = execPromise('composer install --no-dev', {
            cwd: FIXTURES_PATH,
          });
          this.timeout(20000);
  ...
  suite('Bootstrap file for <file> directives', function () {
    ...
    suite('Directory', function () {
      ...
      test('Not matches <file>', async function () {
        const uri = Uri.file(
          path.resolve(FIXTURES_PATH, 'ignore-me/index.php'),
        );
        const run = createRunner(new CancellationTokenSource().token, uri);
        assert.strictEqual(
          (await run.phpcs(testContent)).result.totals.errors,
          0,
        );
      });
    ...
    suite('File', function () {
      ...
      test('Not matches <file>', async function () {
        ...
        {
          const uri = Uri.file(
            path.resolve(FIXTURES_PATH, 'include/class.php'),
          );
          const run = createRunner(new CancellationTokenSource().token, uri);
          assert.strictEqual(
            (await run.phpcs(testContent)).result.totals.errors,
    ...
    suite('Extending', function () {
      ...
      suiteSetup(() =>
        workspace
          ...
          .update(
            'standard',
            './include/bootstrap-file.inherit.xml',
            ConfigurationTarget.Workspace,
          ),
      );
