...
/**
 * Caches file paths that PHPCS has confirmed are excluded by its config
 * (phpcs.xml exclude-patterns). Avoids spawning processes for files that
 * will always be rejected.
 *
 * @type {Set<string>}
 */
const excludedByConfigCache = new Set();

/**
 * Returns true if the file path is known to be excluded by the phpcs config.
 *
 * @param {string} fsPath
 * @return {boolean}
 */
function isExcludedByConfig(fsPath) {
  return excludedByConfigCache.has(fsPath);
}

/**
 * Marks a file path as excluded by the phpcs config.
 *
 * @param {string} fsPath
 */
function markExcludedByConfig(fsPath) {
  excludedByConfigCache.add(fsPath);
}

/**
 * Clears the exclusion cache. Call when phpcs config files change.
 */
function clearExcludedCache() {
  excludedByConfigCache.clear();
}
...
function matchesIgnorePattern(pattern, filePath) {
  ...
  if (pattern.startsWith('/')) {
    // Absolute anchor — normalize separators to forward-slash for cross-platform matching
