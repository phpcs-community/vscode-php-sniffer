  ...
  return {
    async phpcbf(stdin) {
      ...
      if (channel) {
        ...
        log(channel, 'debug', `Command: ${await runFolder}phpcbf ${mapToCliArgs(localArgs, false).join(' ')} -`);
      }
    ...
    async phpcs(stdin) {
      ...
      if (channel) {
        ...
        log(channel, 'info', `Working dir: ${cwd}`);
        log(channel, 'debug', `Command: ${await runFolder}phpcs ${mapToCliArgs(localArgs, false).join(' ')} -q -`);
      }
