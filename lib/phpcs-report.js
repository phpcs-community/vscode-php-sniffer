 ...
 *   The diagnostic.
 */
const messageToDiagnostic =
  (text, tabWidth) =>
  ({ message, line, column, type, source, fixable }) => {
    const textLine = text.split(/\r?\n/)[line - 1];
    const character =
      (textLine.includes('\t')
        ? tabConvert(textLine, tabWidth, column)
        : column) - 1;

    // The ` [fixable]` suffix is the programmatic signal for consumers
    // (e.g. code action providers) to detect fixable violations.
 ...
 */
module.exports.reportFlatten = ({ vscodeOptions, result: { files } }, text) =>
  Object.values(files)
    .reduce(
      (stack, { messages }) => [...stack, ...messages],
      /** @type {PHPCSMessage[]} */ ([]),
    )
