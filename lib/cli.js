...
async function executeCommand(options) {
  const { command, token, stdin = '', spawnOptions = {}, exitCodeThreshold = 0 } = options;
  ...
  if (cliProcess.pid) {
    // @ts-expect-error — stdin is always piped by Node default
  ...
  let stdout = '';
  // @ts-expect-error — stdout is always piped by Node default
  cliProcess.stdout.on('data', (data) => { stdout += data; });

  let stderr = '';
  // @ts-expect-error — stderr is always piped by Node default
  ...
  if (exitCode > exitCodeThreshold) throw new CliCommandError(stdout, stderr, exitCode);
  return stdout;
}
