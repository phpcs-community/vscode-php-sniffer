...
const { markExcludedByConfig } = require('./phpcs-ignore');
  ...
  const disposables = [
    ...
    commands.registerCommand('phpSniffer.runLint', async () => {
      ...
      try {
        ...
        if (
          ...
          markExcludedByConfig(doc.uri.fsPath);
          log(
            channel,
            'debug',
            `phpcs skipped (excluded by config): ${doc.uri.fsPath}`,
          );
        } else {
    ...
    commands.registerCommand('phpSniffer.fixFile', async () => {
      ...
      try {
        ...
        if (fixed != null && fixed !== original) {
          const previewFix = workspace
            .getConfiguration('phpSniffer')
            .get('previewFix', false);
          if (previewFix) {
            const tmpPath = path.join(
              tmpdir(),
              `phpcs-fix-preview-${Date.now()}.php`,
            );
        ...
        if (
          ...
          log(
            channel,
            'debug',
            `phpcbf skipped (excluded by config): ${doc.uri.fsPath}`,
          );
        } else {
          log(channel, 'error', `phpcbf error: ${error.message}`);
          window
            .showErrorMessage('PHP Sniffer: phpcbf failed.', 'Show Output')
            .then((choice) => {
              if (choice) channel.show();
            });
        }
    ...
    commands.registerCommand('phpSniffer.copyDebugInfo', async () => {
      ...
      window.showInformationMessage(
        'PHP Sniffer: Debug info copied to clipboard.',
      );
    }),
  ];
