 ...
 */
const getIndentString = ({ insertSpaces, tabSize }) =>
 ...
 */
const splitEvery = (string, size) =>
  string.match(new RegExp(`.{1,${size}}`, 'g')) || [];
 ...
 */
const getIndentation = (indent) => (lines) => {
  const count = lines.reduce(lintIndentReducer(indent), Number.MAX_SAFE_INTEGER);
  return indent.repeat(count === Number.MAX_SAFE_INTEGER ? 0 : count);
};
...
const createIndentProcessor = (formatOptions) => {
  ...
  return {
    ...
    unprocess(string) {
      return indent ? string.replace(/^(.+)/gm, `${indent}$1`) : string;
    },
  };
};
