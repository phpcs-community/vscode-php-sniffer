  ...
  return {
    async phpcbf(stdin) {
      ...
      if (channel) {
        ...
        log(channel, 'debug', `Command: ${await runFolder}phpcbf ${mapToCliArgs(localArgs, !!spawnOptions.shell).join(' ')} -`);
      }
      ...
      if (!phpcbfResult) return null; // cancelled
      ...
      return phpcbfResult.stdout || stdin;
    },
    async phpcs(stdin) {
      const localArgs = new Map(args);
      localArgs.set(
      ...
      if (channel) {
        ...
        log(channel, 'info', `Working dir: ${cwd}`);
        log(channel, 'debug', `Command: ${await runFolder}phpcs ${mapToCliArgs(localArgs, !!spawnOptions.shell).join(' ')} -q -`);
      }
      ...
      try {
        ...
        throw new Error(`phpcs returned non-JSON output: ${result.stdout.slice(0, 200)}`);
      }
