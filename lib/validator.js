 ...
 */
const logError = (channel, message) => {
 ...
 */
const validateDocument = (diagnostics, tokenManager, channel) => (document) => {
  ...
  resultPromise
    ...
    .catch((error) => {
      if (error.message === 'spawn phpcs ENOENT') {
        logError(
          channel,
          'phpcs not found in PATH. Install phpcs globally or set `phpSniffer.executablesFolder`.',
        );
      } else if (error.message.endsWith(' ENOENT')) {
        const msg =
          'Specified `phpcs` executable not found, please check your `executableFolder` setting.';
        logError(channel, msg);
        window.showErrorMessage(msg);
      } else {
        logError(channel, `phpcs error: ${error.message}`);
...
module.exports.createValidator = (channel) => {
  ...
  const validate = validateDocument(diagnostics, tokenManager, channel);
