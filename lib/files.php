...
foreach ($this->ruleset->paths as $ruleset_path) {
    ...
    foreach ($ruleset->file as $file_path) {
      ...
      // Use directory-boundary check to avoid false positives (e.g. /app
      // matching /app2/file.php). Accept exact match or path under directory.
        if ($full_path && ($processing_file === $full_path || strpos($processing_file, $full_path . DIRECTORY_SEPARATOR) === 0 || strpos($processing_file, $full_path . '/') === 0)) {
            return;
        }
    }
}
