    ...
    async phpcbf(stdin) {
      ...
      if (channel) {
        ...
        log(
          channel,
          'debug',
          `Command: ${await runFolder}phpcbf ${mapToCliArgs(localArgs, !!spawnOptions.shell).join(' ')} -q -`,
        );
      }
      ...
      const phpcbfResult = await executeCommand({
        ...
        token,
        args: [...mapToCliArgs(localArgs, !!spawnOptions.shell), '-q', '-'],
        stdin,
        spawnOptions,
        exitCodeThreshold: 2,
        timeout,
      ...
      try {
        ...
        if (error.stdout) {
          try {
            ...
            if (parsed && parsed.totals !== null && typeof parsed.totals === 'object') {
              let tabWidth = 1;
              try {
