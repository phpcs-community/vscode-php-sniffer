 ...
 * @property {import('vscode').DiagnosticCollection} diagnostics
 *   The diagnostic collection managed by this validator.
 */
...
const shouldValidate = (document) => {
  ...
  return (
    ...
    !isIgnoredByPhpcs(document.uri.fsPath) &&
    !isExcludedByConfig(document.uri.fsPath)
  );
};
...
const validateDocument =
  (diagnostics, tokenManager, channel, queue, onDiagnosticsSet) =>
  (document) => {
    ...
    const resultPromise = queue.run(() =>
      createRunner(token, document.uri, true, channel).phpcs(text),
    );
    ...
    resultPromise
      ...
      .catch((error) => {
        if (error.code === 'ENOENT') {
          ...
          // v3
          (error.message || '').includes('No violations were found')
        ) {
          // File is excluded by phpcs.xml exclude-pattern — cache it to skip future runs.
          markExcludedByConfig(document.uri.fsPath);
          log(
            channel,
            'debug',
            `phpcs skipped (excluded by config): ${document.uri.fsPath}`,
          );
        } else {
          log(channel, 'error', `phpcs error: ${error.message}`);
          window
            .showErrorMessage('PHP Sniffer: phpcs failed.', 'Show Output')
            .then((choice) => {
              if (choice) channel.show();
            });
        }
...
module.exports.createValidator = (channel) => {
  ...
  const resetValidator = reset.bind(
    null,
    diagnostics,
    tokenManager,
    validatorRuntime,
  ...
  // Watch phpcs config files — any change triggers a full reset so rules and
  // exclude-patterns are re-evaluated for all open documents.
  const phpcsConfigWatcher = workspace.createFileSystemWatcher(
    '**/{phpcs.xml,.phpcs.xml,phpcs.xml.dist,.phpcs.xml.dist}',
  );
  phpcsConfigWatcher.onDidChange(resetValidator);
  phpcsConfigWatcher.onDidCreate(resetValidator);
  phpcsConfigWatcher.onDidDelete(resetValidator);

  const workspaceListeners = [
    phpcsConfigWatcher,
    workspace.onDidChangeConfiguration(isRunAffect(resetValidator)),
    ...
    workspace.onDidChangeWorkspaceFolders(resetValidator),
  ];
