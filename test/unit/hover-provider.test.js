  ...
  suite('provideHover()', function () {
    ...
    test('Returns Hover with rule name when PHPCS diagnostic is at position', function () {
      const diagnostics = {
        get: () => [
          makeDiagnostic({
            source: 'PHPCS',
            code: 'PSR2.ControlStructures.SwitchDeclaration.SpaceBeforeColon',
      ...
      ok(
        content.includes(
          'PSR2.ControlStructures.SwitchDeclaration.SpaceBeforeColon',
        ),
    ...
    test('Includes fixable hint when message ends with [fixable]', function () {
      const diagnostics = {
        get: () => [
          makeDiagnostic({
            source: 'PHPCS',
            code: 'Generic.Formatting.SpaceAfterCast',
            message: 'Expected 1 space after cast [fixable]',
            rangeContains: () => true,
          }),
        ],
      };
      ...
      const content = result.contents[0].value;
      ok(
        content.includes('auto-fixed with PHPCBF'),
        'should include fixable hint',
      );
    });
    ...
    test('Does not include fixable hint when message does not end with [fixable]', function () {
      ...
      strictEqual(
        content.includes('auto-fixed with PHPCBF'),
        false,
        'should not include fixable hint',
      );
    });
    ...
    test('Shows all overlapping diagnostics when multiple PHPCS diagnostics overlap the position', function () {
      const diagnostics = {
        get: () => [
          ...
          makeDiagnostic({
            source: 'PHPCS',
            code: 'Rule.Two',
            message: 'Second violation [fixable]',
            rangeContains: () => true,
          }),
        ],
      };
      ...
      ok(
        content.includes('---'),
        'should separate diagnostics with a horizontal rule',
      );
      ok(
        content.includes('auto-fixed with PHPCBF'),
        'should include fixable hint for second',
      );
    });
  });
});
