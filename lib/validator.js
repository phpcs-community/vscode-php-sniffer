 ...
 */
const validateDocument = (diagnostics, tokenManager, channel, queue) => (document) => {
  ...
  const resultPromise = queue.run(() => createRunner(token, document.uri).phpcs(text));
...
module.exports.createValidator = (channel) => {
  ...
  const validate = validateDocument(diagnostics, tokenManager, channel, queue);
